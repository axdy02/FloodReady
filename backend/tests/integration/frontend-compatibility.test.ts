...
describe("frontend compatibility", () => {
  it("enforces map bounds, privacy, cursor binding, rejected exclusion, totals, and rate limits", async () => {
    ...
    expect(Object.keys(page.body.data.items[0]).sort()).toEqual(["canViewDetails", "capturedAt", "category", "id", "incidentId", "latitude", "longitude", "severityClaim", "submittedAt", "updatedAt", "verificationStatus"])
  ...
  it("preserves legacy DEVICE_GPS and enforces manual accuracy and description boundaries", async () => {
    ...
    expect((await submitReport(actor.accessToken, { description: "😀".repeat(500) })).status).toBe(201)
    expect((await submitReport(actor.accessToken, { description: "😀".repeat(501) })).status).toBe(400)
