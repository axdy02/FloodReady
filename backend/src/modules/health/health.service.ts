...
const data = (status: "ok" | "ready"): HealthData => ({ status, service: "waterradar-backend", timestamp: new Date().toISOString() });
...
export const readiness = async (): Promise<HealthData> => {
  ...
  try {
    await Promise.all([probeDatabase(), imageStorage.checkHealth()]);
