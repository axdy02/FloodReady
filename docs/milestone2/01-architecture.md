...
Verified against the repository state on 2026-07-19. The canonical user-facing application is `frontend/` on port 3000. `wireframe/` is a second comparison/testing frontend on port 3002 and retains the older draft-review flow.
...
flowchart LR
  ...
  V --> S[("Private image storage\nlocal volume or private S3")]
...
| Private image storage | `IMAGE_STORAGE_DRIVER=local` uses `UPLOAD_DIRECTORY`; `IMAGE_STORAGE_DRIVER=s3` uses private S3 through the AWS SDK default credential chain. Both store bytes under `reports/<report-id>/<UUID>.<ext>`. | Database stores the opaque `image_path` and image metadata; bytes are not in PostgreSQL and no public object URL is issued. |
...
3. Backend 1 validates the file signature/MIME, decodes it with Sharp, applies EXIF rotation, re-encodes it, and saves the processed bytes under an opaque key in local storage or private S3.
...
6. Backend 1 reads the just-stored image through `ImageStorage`, then schedules `processReportAnalysis` with `queueMicrotask`. It passes those stored bytes, report ID, analysis ID, description, user severity, coordinates, MIME, and the original request ID to Backend 2.
...
- There is no signed URL, public object access, malware scanner, duplicate rejection, or scheduled stale-image cleanup. S3 is an optional private driver for EC2.
...
- Image pipeline: `backend/src/shared/storage/image-processor.ts`, `image-storage-factory.ts`, `local-image-storage.ts`, `s3-image-storage.ts`, `docker-compose.yml`, `docker-compose.ec2.yml`
