...
describe("environment validation", () => {
  ...
  it("rejects missing, malformed, and cross-origin values", () => {
    ...
    expect(() => parseClientEnvironment({ ...source, FRONTEND_ENV: "production", NEXT_PUBLIC_CARTO_BASEMAP_KEY: undefined, NEXT_PUBLIC_API_BASE_URL: "https://api.example.invalid/api/v1", NEXT_PUBLIC_APP_ORIGIN: "https://app.example.invalid" })).toThrow("NEXT_PUBLIC_CARTO_BASEMAP_KEY is required in production");
  });
