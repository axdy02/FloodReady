...
const aiAnalysisStatusSchema = z.enum(["PROCESSING", "SUCCEEDED", "FAILED", "TIMED_OUT"]);
const aiAnalysisSchema = z.object({ id: z.uuid(), status: aiAnalysisStatusSchema, floodDetected: z.boolean().nullable(), suggestedSeverity: severitySchema.nullable(), confidenceScore: z.number().min(0).max(1).nullable(), validationScore: z.number().min(0).max(1).nullable(), validationOutcome: z.enum(["ACCEPTED", "NEEDS_REVIEW", "REJECTED"]).nullable(), weatherSummary: z.string().nullable(), weatherPrecipitationMm: z.number().nullable(), weatherTemperatureC: z.number().nullable(), waterLevelCategory: z.string().nullable(), roadPassability: z.string().nullable(), imageQuality: z.string().nullable(), summary: z.string().nullable(), evidenceFlags: z.array(z.string()), needsHumanReview: z.boolean().nullable(), modelName: z.string().nullable(), modelVersion: z.string().nullable(), processingTimeMs: z.number().int().nullable() }).strict();
const mapAiAnalysisSchema = aiAnalysisSchema.pick({ status: true, floodDetected: true, suggestedSeverity: true, confidenceScore: true, validationScore: true, validationOutcome: true, needsHumanReview: true });
