import { authDtoSchema, backendHealthSchema, incidentPageSchema, reportDtoSchema, reportMapPageSchema, reportPageSchema, servicesHealthSchema, userDtoSchema, userPageSchema } from "@/lib/api/contracts";
import { request } from "@/lib/api/request";

export const api = {
  ready: () => request({ method: "GET", path: "/health/ready", schema: backendHealthSchema }),
  services: (accessToken: string) => request({ method: "GET", path: "/health/services", accessToken, schema: servicesHealthSchema }),
