...
const report = { id: "20000000-0000-4000-8000-000000000001", reporterId: validUser.id, category: "FLOODED_ROAD", description: "Water covers one lane.", severityClaim: "MODERATE", finalSeverity: "MODERATE", aiUsed: false, aiAnalysis: null, latitude: 0, longitude: 0, gpsAccuracy: null, locationSource: "MANUAL", capturedAt: "2026-01-01T00:00:00.000Z", submittedAt: "2026-01-01T00:00:00.000Z", uploadSource: "WEB", verificationStatus: "SUBMITTED", incidentId: null, createdAt: "2026-01-01T00:00:00.000Z", updatedAt: "2026-01-01T00:00:00.000Z" } as const;
...
const mapPage = { items: [{ id: report.id, category: report.category, severityClaim: report.severityClaim, finalSeverity: report.finalSeverity, aiUsed: report.aiUsed, aiAnalysis: null, latitude: 0, longitude: 0, capturedAt: report.capturedAt, submittedAt: report.submittedAt, verificationStatus: report.verificationStatus, incidentId: null, updatedAt: report.updatedAt, canViewDetails: true }], pagination: { limit: 20, hasMore: false, nextCursor: null }, totalCount: 1 } as const;
...
describe("typed API boundary", () => {
  ...
  it("supports protected image responses without exposing a URL", async () => {
    const fetchMock = vi.spyOn(globalThis, "fetch").mockResolvedValue(new Response(new Uint8Array([1, 2]), { status: 200, headers: { "Content-Type": "image/jpeg", "Content-Length": "2", "Cache-Control": "private, no-store" } }));
    const result = await reportsApi.image(report.id, "token");
    expect(result.mime).toBe("image/jpeg");
    expect(result.blob.size).toBe(2);
    const [url, init] = fetchMock.mock.calls[0] ?? [];
    expect(String(url)).toBe(`http://localhost:3001/api/v1/reports/${report.id}/image`);
