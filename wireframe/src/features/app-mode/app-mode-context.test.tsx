...
function ModeProbe() {
  ...
  return <><output>{mode}</output><button type="button" onClick={() => setMode(mode === "live" ? "demo" : "live")}>Toggle mode</button></>;
}
...
describe("wireframe display adapters", () => {
  it("uses a safe live fallback and lets the presentation toggle its isolated mode", () => {
    render(<ModeProbe />);
  ...
  it("maps live reports and incidents to each visible severity and lifecycle tone", () => {
    ...
    expect(incidentToDisplayAlert(incident).level).toBe("High");
  });
});
